    ...
    void RenderDesignSelector(ClusterApp::appModel& model, ClusterApp::options& option){
        ...
        if(ImGui::BeginPopupModal("Parallel menu", &parallel_modal, ImGuiWindowFlags_AlwaysAutoResize)){
            ...
            if (ImGui::Button("Next")) {
                ...
                model.first_design_click = true;
                ImGui::CloseCurrentPopup();
            } ImGui::SameLine();
            ImGui::PopID();
            ImGui::PushID(933820+model.id);
            if (ImGui::Button("Close")) {
                ImGui::CloseCurrentPopup();
            }
            ImGui::PopID();
        ...
        if(ImGui::BeginPopupModal("Factorial menu", &factorial_modal, ImGuiWindowFlags_AlwaysAutoResize)){
            if (option.two_treatments) {
                ...
                ImGui::Text("To enable this option, set two treatments in Global Options.");
            }
            ImGui::PopID();
            ImGui::EndPopup();
        }
...
void RenderModelSelector(ClusterApp::appModel& model, ClusterApp::options& option){
    ImGui::Begin(int_to_char("Select Model ",model.id+1));
    ...
    ImGui::Text("Statistical model");
        ...
        if(!model.option.use_exact_cell_sizes){
            ...
            ImGui::SetNextItemWidth(100);
            ImGui::InputInt("Cluster-period size", &model.model.mean_size, 1, 1000, 0); 
            ImGui::Text("You can set the sizes of specific sequences and periods in the designer");
        }
    ...
    model.model.ind_covariance = static_cast<ClusterApp::IndividualCovariance>(ind_cov_item_current + 1);
        ...
        if (!option.two_treatments) {
            ImGui::SameLine(); HelpMarker("Automatically set fixed effect parameters by specifying the mean outcomes in treatment and control groups. Temporal variation is assumed to be zero. For non-linear models, a correction is applied for the random effect. Parameter values can be found above.");
                ...
                case ClusterApp::Family::gaussian: // case ClusterApp::Family::quantile:
                    ImGui::SetNextItemWidth(200);
                    ImGui::DragFloat("Control group mean", &model.model.treat_control_means[0], 0.01f, -FLT_MAX, +FLT_MAX, "%.3f", ImGuiSliderFlags_None);
                    ImGui::SetNextItemWidth(200);
                    ImGui::DragFloat("Treatment group mean", &model.model.treat_control_means[1], 0.01f, -FLT_MAX, +FLT_MAX, "%.3f", ImGuiSliderFlags_None);
                    break;
                case ClusterApp::Family::binomial: case ClusterApp::Family::beta:
                    ImGui::SetNextItemWidth(200);
                    ImGui::DragFloat("Control group mean", &model.model.treat_control_means[0], 0.01f, 0.0f, 1.0f, "%.3f", ImGuiSliderFlags_None);
                    ImGui::SetNextItemWidth(200);
                    ImGui::DragFloat("Treatment group mean", &model.model.treat_control_means[1], 0.01f, 0.0f, 1.0f, "%.3f", ImGuiSliderFlags_None);
                    break;
                case ClusterApp::Family::poisson: case ClusterApp::Family::gamma:
                    ImGui::SetNextItemWidth(200);
                    ImGui::DragFloat("Control group mean", &model.model.treat_control_means[0], 0.01f, 0.0f, +FLT_MAX, "%.3f", ImGuiSliderFlags_None);
                    ImGui::SetNextItemWidth(200);
                    ImGui::DragFloat("Treatment group mean", &model.model.treat_control_means[1], 0.01f, 0.0f, +FLT_MAX, "%.3f", ImGuiSliderFlags_None);
                    break;
                }
        ...
        if(ImGui::Button("Next")){
            if(model.model.family != ClusterApp::Family::gaussian){
                float re_adj = 0; //model.model.cov_pars[0]; // set to zero for now
                ...
                case ClusterApp::Link::identity:
                {
                    if (model.model.include_intercept == 1) {
                        model.model.beta_pars[0] = model.model.treat_control_means[0];
                    }
                    else {
                        for (int l = 0; l < model.model.beta_pars.size(); l++) {
                            model.model.beta_pars[l] = model.model.treat_control_means[1];
                        }
                    }
                    model.model.te_pars[0] = model.model.treat_control_means[1] - model.model.treat_control_means[0];
                    break;
                }
                case ClusterApp::Link::log:
                {
                    if (model.model.include_intercept == 1) {
                        model.model.beta_pars[0] = log(model.model.treat_control_means[0]) - re_adj;
                    }
                    else {
                        for (int l = 0; l < model.model.beta_pars.size(); l++) {
                            model.model.beta_pars[l] = log(model.model.treat_control_means[0]) - re_adj;
                        }
                    }
                    model.model.te_pars[0] = log(model.model.treat_control_means[1]) - log(model.model.treat_control_means[0]);
                    break;
                }
                case ClusterApp::Link::logit:
                {
                    if (model.model.include_intercept == 1) {
                        model.model.beta_pars[0] = log(model.model.treat_control_means[0] / (1 - model.model.treat_control_means[0])) - re_adj;
                    }
                    else {
                        for (int l = 0; l < model.model.beta_pars.size(); l++) {
                            model.model.beta_pars[l] = log(model.model.treat_control_means[0] / (1 - model.model.treat_control_means[0])) - re_adj;
                        }
                    }
                    model.model.te_pars[0] = log(model.model.treat_control_means[1] / (1 - model.model.treat_control_means[1])) - log(model.model.treat_control_means[0] / (1 - model.model.treat_control_means[0]));
                    break;
                }
                case ClusterApp::Link::probit:
                {
                    ...
                    if (model.model.include_intercept == 1) {
                        model.model.beta_pars[0] = boost::math::quantile(norm, model.model.treat_control_means[0]) - re_adj;
                    }
                    else {
                        for (int l = 0; l < model.model.beta_pars.size(); l++) {
                            model.model.beta_pars[l] = boost::math::quantile(norm, model.model.treat_control_means[0]) - re_adj;
                        }
                    }
                    model.model.te_pars[0] = boost::math::quantile(norm, model.model.treat_control_means[1]) - model.model.beta_pars[0];
                    break;
                }
                case ClusterApp::Link::inverse:
                {
                    if (model.model.include_intercept == 1) {
                        model.model.beta_pars[0] = 1/model.model.treat_control_means[0];
                    }
                    else {
                        for (int l = 0; l < model.model.beta_pars.size(); l++) {
                            model.model.beta_pars[l] = 1/model.model.treat_control_means[0];
                        }
                    }
                    model.model.te_pars[0] = 1/(model.model.treat_control_means[1] - model.model.treat_control_means[0]);
                    break;
                }
            ...
            if(model.option.use_exact_cell_sizes){
                for (int i = 0; i < model.designs.sequences; i++){
                    for(int t = 0; t < model.designs.time; t++){
                        *model.designs.n(i, t) = model.model.mean_size;
                    }
                }
            }
            model.checker.update();
            model.first_time = false;
            model.first_model_click = true;
        }
    ...
    void RenderMenuBar(ClusterApp::appModel& model, ClusterApp::options& option, bool* mopen) {
        ImGui::Begin(int_to_char("Design ",model.id+1), mopen);
            ...
            ImGui::SeparatorText("Menu");
            {
                ...
                if (ImGui::Button("Change trial design")){
                ...
                ImGui::SameLine();
                ImGui::SetNextWindowPos(ImVec2((model.id*50)+185, 60+(model.id*50)), ImGuiCond_FirstUseEver);
                ...
                if(model.first_time && model.first_design_click) model.modelopen = true;
                if (ImGui::Button("Change statistical model")){
                ...
                ImGui::SetNextWindowPos(ImVec2((model.id*50)+185, 50+(model.id*50)), ImGuiCond_FirstUseEver);
                if (ImGui::BeginPopupModal("Choose stat model", &model.modelopen, ImGuiWindowFlags_AlwaysAutoResize))
                {
                    ...
                    if(!model.option.use_exact_cell_sizes){
                        ...
                        ImGui::SetNextItemWidth(100);
                        ImGui::InputInt("Cluster-period size", &model.model.mean_size, 1, 1000, 0); ImGui::SameLine();
                        if(ImGui::Button("Set size of all cluster-periods")){
                            for (int i = 0; i < model.designs.sequences; i++){
                                for(int t = 0; t < model.designs.time; t++){
                                    *model.designs.n(i, t) = model.model.mean_size;
                                }
    ...
    // trial design graphical
            ...
            ImGui::SeparatorText("Results");
            {
                ...
                if (!option.auto_update && model.updater.requires_update && !model.updater.is_updating){
                    ...
                    {
                        if (ImGui::BeginTabItem("Power, SE, and other stats"))
                            {
                            ...
                            if (ImGui::BeginTabItem("Find sample sizes"))
                            {
