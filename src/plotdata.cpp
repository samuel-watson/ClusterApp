...
void ClusterApp::plotData::extract_y(ClusterApp::modelSummary& summary, int i, int s) {
	...
	switch (s) {
	case 0:
		y_data_1[i] = yaxis == ClusterApp::YAxis::power ? summary.power : summary.min_eff;
		break;
	case 1:
		y_data_2[i] = yaxis == ClusterApp::YAxis::power ? summary.power : summary.min_eff;
		break;
	case 2:
		y_data_3[i] = yaxis == ClusterApp::YAxis::power ? summary.power : summary.min_eff;
		break;
	}
